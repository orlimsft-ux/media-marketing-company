...
export function runEditorAgent(db) {
  ...
  for (const project of normalized.editingProjects) {
    ...
    const hasMaterial = project.materialStatus === 'ready' || Boolean(project.sourceFile) || normalized.materialQueue.some(item => item.status === 'downloaded');
    const brief = createEditorBrief(video, project, hasMaterial);

    project.editorBrief = brief;
    project.version = project.version || 'v1';
    ...
    project.nextAction = hasMaterial
      ? '剪辑 Agent 已生成脚本、分镜和素材清单，等待老板审核后进入粗剪。'
    ...
    if (task) {
      ...
      task.summary = hasMaterial
        ? `二创剪辑 Agent 已为「${video?.title || project.title}」生成可审核剪辑方案。`
        : `二创剪辑 Agent 已生成预案，但「${video?.title || project.title}」仍缺少可剪素材。`;
      task.deliverable = formatEditorDeliverable(brief, hasMaterial);
      task.updatedAt = now;
    }
  ...
  const summary = updatedProjects.length
    ? `二创剪辑 Agent 已生成 ${updatedProjects.length} 个剪辑方案，等待老板审核。`
...
function formatEditorDeliverable(brief, hasMaterial) {
  return [
    `开头：${brief.hook}`,
    `角度：${brief.angle}`,
    ...
    `文案：${brief.caption}`,
    `风险：${brief.riskCheck}`,
    hasMaterial ? '状态：素材已就绪，可进入粗剪。' : '状态：等待素材采集 Agent 提供源文件。'
  ].join('\n');
}
