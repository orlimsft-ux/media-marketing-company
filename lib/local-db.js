...
const sourceVideoDir = process.env.VERCEL
  ? path.join(runtimeDataDir, 'source-videos')
  : path.join(process.cwd(), 'assets', 'source-videos');
...
export async function addUploadedTestVideo(db, file) {
  const normalized = normalizeDb(db);
  const now = new Date().toISOString();
  const targetName = `${Date.now()}-${safeAssetName(file.name, 'test-video.mp4')}`;
  const targetPath = path.join(sourceVideoDir, targetName);
  const projectId = `edit_upload_${Date.now()}`;
  const taskId = `task_upload_edit_${Date.now()}`;
  const title = `测试视频二创：${path.basename(file.name, path.extname(file.name)) || '上传视频'}`;

  await mkdir(sourceVideoDir, { recursive: true });
  await writeFile(targetPath, Buffer.from(file.arrayBuffer));

  const localPath = relativeAssetPath(targetPath);
  const materialItem = {
    id: `material_upload_${Date.now()}`,
    platform: '测试视频',
    title,
    sourceUrl: '',
    status: 'downloaded',
    ownerId: 'agent_editor',
    note: '老板上传的测试视频已进入素材库，可直接交给二创剪辑 Agent 生成版本。',
    localPath,
    createdAt: now,
    updatedAt: now
  };

  normalized.materialQueue.unshift(materialItem);
  normalized.editingProjects.unshift({
    id: projectId,
    taskId,
    sourceFile: localPath,
    title,
    status: 'in_progress',
    goal: '用上传视频测试二创剪辑 Agent：生成 1-3 个版本，并交给推广 Agent。',
    version: 'v1',
    nextAction: '测试视频已入库，运行剪辑 Agent 生成二创版本。',
  ...
  normalized.tasks.unshift({
    ...
    priority: 'high',
    dueAt: now,
    summary: `测试视频已上传：${file.name}。等待二创剪辑 Agent 生成版本。`,
    deliverable: '等待二创剪辑 Agent 输出 1-3 个成片版本和推广交接包。',
    createdAt: now
  });
...
function createEditVersions(project, brief, hasMaterial) {
  ...
  const baseOutput = project.sourceUrl ? project.id : exportBaseName(project.sourceFile || project.id, project.id);
...
function findSourceForProject(db, project) {
  ...
  if (project.sourceFile) {
    return db.materialQueue.find(item => item.localPath === project.sourceFile)
      || { status: 'downloaded', localPath: project.sourceFile };
  }
...
function exportBaseName(name, fallback) {
  const ext = path.extname(name) || path.extname(fallback);
  return path.basename(safeAssetName(name, fallback), ext).replace(/^-+|-+$/g, '') || path.basename(fallback, ext);
}
